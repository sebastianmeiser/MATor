...
class MATor : public std::enable_shared_from_this<MATor>
{
	public:
		...
		MATor(
			...
			std::shared_ptr<PathSelectionSpec> pathSelectionSpec1,
			std::shared_ptr<PathSelectionSpec> pathSelectionSpec2,
			std::string& consensus, std::string& database = emptystring, bool fast = false)
			: MATor(senderSpec1, senderSpec2, recipientSpec1, recipientSpec2, pathSelectionSpec1, pathSelectionSpec2, Config(consensus, database, fast)){}
