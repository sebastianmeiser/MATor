...
class Config : public std::enable_shared_from_this<Config>
{
	public:
		...
		Config(std::string& consensusFile, std::string& databaseFile = emptystring, bool fast = false, bool precompute = false, double epsilon = 1)
			: consensusFile(consensusFile), databaseFile(databaseFile), fast(fast), precompute(precompute), epsilon(epsilon){}
		Config(){}
};
