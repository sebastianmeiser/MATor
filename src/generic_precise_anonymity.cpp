	...
	for (int obstaskindex = 0; obstaskindex < 3; obstaskindex++)
	{
		for(size_t i = 0; i < size; i += chunk_size)
		{
			...
			manager.addTask([&, begin, end, obstaskindex](){
				const_vector<numeric_type> SAObsProbsA1(4, 0);
				const_vector<numeric_type> SAObsProbsB1(4, 0);
				const_vector<numeric_type> RAObsProbsA1(4, 0);
				const_vector<numeric_type> RAObsProbsA2(4, 0);
				const_vector<numeric_type> RELObsProbsA1(4, 0);
				const_vector<numeric_type> RELObsProbsA2(4, 0);
				const_vector<numeric_type> RELObsProbsB1(4, 0);
				const_vector<numeric_type> RELObsProbsB2(4, 0);
 	...
 	// Run prepared jobs:
	clogsn("Main loop starts.");
	manager.startAndJoinAll();
	clogsn("Main loop done.");
