...
PYBIND11_PLUGIN(pymator) {
	...
	py::class_<MATor, shared_ptr<MATor>>(m, "MATor")
		...
		.def(py::init<std::shared_ptr<SenderSpec>, std::shared_ptr<SenderSpec>, std::shared_ptr<RecipientSpec>, std::shared_ptr<RecipientSpec>, std::shared_ptr<PathSelectionSpec>, std::shared_ptr<PathSelectionSpec>, std::string&, std::string&, bool>())
	...
	py::class_<Consensus, shared_ptr<Consensus>>(m, "Consensus")
		.def("__init__", [](Consensus &instance, string& consensus, string& dbname) {
				py::gil_scoped_release release;
				new (&instance) Consensus(consensus, dbname);
		})
