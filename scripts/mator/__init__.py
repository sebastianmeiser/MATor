...
class MATorConfig(object):
	...
	def __init__(self, senderSpec1, senderSpec2, recipientSpec1, recipientSpec2, psSpec1, psSpec2, fast = False):
	...
	def createMATor(self, consensus):
		if isinstance(consensus, MATorConsensus):
			m = MATor(self.senderSpec1, self.senderSpec2, self.recipientSpec1, self.recipientSpec2, self.psSpec1, self.psSpec2, consensus.consensus, consensus.database, self.fast)
...
class MATorConsensus(object):
	def __init__(self, consensus, database = ""):
	...
	def __ne__(self, other):
		return not self.__eq__(other)
	def load(self):
		return Consensus(self.consensus, self.database)
